...
import { Focus } from '../../common-behaviors/active-index';
import { useId } from '../../common-behaviors/use-id';
import { MenuItemDataRef } from '../types/menu-item';
...
@Component({
  ...
  template: `
    <ng-template
      ...
      [ngTemplateOutletContext]="{ $implicit: dataRef, active: active, disabled: _disabled }"
    ></ng-template>
...
export class MenuItemComponent implements AfterViewInit, OnDestroy {
  ...
  get active(): boolean {
    return this.menu.activeItemIndex !== null
      ? this.menu.items[this.menu.activeItemIndex].id === this.id
      : false;
  }
