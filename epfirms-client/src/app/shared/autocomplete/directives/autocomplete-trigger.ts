...
import { _AutocompleteOriginBase } from './autocomplete-origin';

/** Injection token that determines the scroll handling while the autocomplete panel is open. */
export const AUTOCOMPLETE_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'ep-autocomplete-scroll-strategy'
);
...
export function getAutocompleteMissingPanelError(): Error {
  return Error(
    'Attempting to open an undefined instance of `ep-autocomplete`. ' +
      'Make sure that the id passed to the `epAutocomplete` is correct and that ' +
      "you're attempting to open it after the ngAfterContentInit hook."
  );
}

/** Base class with all of the `AutocompleteTrigger` functionality. */
@Directive()
export abstract class _AutocompleteTriggerBase
  implements ControlValueAccessor, AfterViewInit, OnChanges, OnDestroy
{
  ...
  private _previousSelection: string | number | null;
  ...
  @Input('epAutocomplete') autocomplete: _AutocompleteBase;
   ...
   */
  // eslint-disable-next-line @angular-eslint/no-input-rename
  @Input('epAutocompletePosition') position: 'auto' | 'above' | 'below' = 'auto';

  /**
   * Reference relative to which to position the autocomplete panel.
   * Defaults to the autocomplete trigger element.
   */
  @Input('epAutocompleteConnectedTo') connectedTo: _AutocompleteOriginBase;
  ...
  constructor(
    ...
    @Optional()
    @Inject(AUTOCOMPLETE_DEFAULT_OPTIONS)
    private _defaults?: AutocompleteDefaultOptions
  ) {
  ...
  /** The currently active option, coerced to OptionComponent type. */
  ...
  _handleFocus(): void {
    if (!this._canOpenOnNextFocus) {
      ...
      this._element.nativeElement.placeholder = this._element.nativeElement.value;
      this._previousSelection = this._element.nativeElement.value;
  ...
  private _setTriggerValue(value: any): void {
    ...
    this._previousSelection = inputValue;
  }
  ...
  private _setValueAndClose(event: OptionSelectionChange | null): void {
    if (event && event.source) {
      if (event.isUserInput) {
        this._resetValueAndPlaceholder();
      }
  ...
  private _resetValueAndPlaceholder(): void {
    this._element.nativeElement.value = this._previousSelection.toString();
...
@Directive({
  selector: `input[epAutocomplete], textarea[epAutocomplete]`,
  host: {
    class: 'ep-autocomplete-trigger',
  ...
  exportAs: 'epAutocompleteTrigger',
  providers: [AUTOCOMPLETE_VALUE_ACCESSOR]
})
// eslint-disable-next-line @angular-eslint/directive-class-suffix
export class AutocompleteTrigger extends _AutocompleteTriggerBase {
  protected _aboveClass = 'ep-autocomplete-panel-above';
}
