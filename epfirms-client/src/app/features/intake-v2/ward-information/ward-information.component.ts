...
export class WardInformationComponent implements OnInit {
  ...
  // note that in this case, the client is the ward user profile
  client;

  //ward holds the record from the DB about the ward and the matter
  ward : Ward;
  ...
  private init(): void {
    // check to see if there is a ward record assosciated with the matter id

    this.wardService.getWardWithMatterId(this.matter.id).subscribe((ward) => {
      if (ward) {
        this.ward = ward;

        // if there is a ward record, make the call to the user service to get the ward user profile
        this.userService.get(ward.user_id).subscribe((user) => {
          if (user) {
            this.client = user;
            this.loadClientData();
        ...
        // if there is no ward record, make the new user, and then create ward record
          ...
          if (createdUser) {
            ...
            this.wardService
              .upsert({
                ...
                caregiver_id: this.matter.client.id,
              })
              .subscribe((createdward) => {
                if (createdward) {
                  this.ward = createdward;
                }
              });
          }
        });
